...
class SpiIntegrationTest {
  ...
  void test_valuePreprocessor_transforms_input_data() {
    ...
    ValuePreprocessor preprocessor = row -> {
      ...
      processedRow.forEach((key, value) -> {
        if ("YES".equals(value)) {
    ...
    MappingConfig cfg = MappingConfig.builder()
        .separator("/")
  ...
  void test_reporter_captures_warnings() {
    ...
    assertThat(warnings.getFirst()).contains("keyPath(s) [definitions/tracker/tasks/taskDate] are missing or null");
  ...
  void test_reporter_captures_conflict_warnings() {
    ...
    assertThat(warnings).anyMatch(warning ->
        warning.contains("lastWriteWins policy") &&
        warning.contains("definitions/name"));
