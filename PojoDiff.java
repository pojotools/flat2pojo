...
public final class PojoDiff {
    ...
    public static <T> List<Entry> diffList(
        ...
        // When diffing a root list, strip the first dynamic segment for type resolution (/C-1/...)
        tmp.typeHintBaseDepth(1);

        // If no hints provided and auto-infer enabled, infer from first non-null element
        if (tmp.autoInferTypes && tmp.typeHints == null) {
            Class<?> clazz = firstNonNullClass(left, right);
            if (clazz != null) tmp.typeHints(inferTypeHints(clazz, mapper));
        }

        List<Entry> diffs = diff(L, R, tmp);
        // drop "/__root" prefix from paths
    ...
    public static <T> List<Entry> diffMap(
        ...
        Config tmp = cfg.copy();
        // For maps, paths start with /<key>/..., so strip that first segment for type resolution
        tmp.typeHintBaseDepth(1);
    ...
    public static final class Config {
        ...
        // How many leading segments to strip when resolving type hints (e.g., 1 for /<rootKey>/...)
        private int typeHintBaseDepth = 0;
    ...
    private static JavaType resolveTypeWithWildcards(JsonPointer fullPath, TypeHints hints, int baseDepth) {
        ...
        // 3) Replace trailing segments with "/*" progressively
        ...
        // 4) If nothing matched, try fully wildcarded path of same depth
