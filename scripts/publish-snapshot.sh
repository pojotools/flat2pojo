...
# flat2pojo SNAPSHOT Publishing Script
...
# Configuration
MODULE="flat2pojo-jackson"
PROFILE="snapshot-deploy"

echo -e "${BLUE}===== flat2pojo SNAPSHOT Publisher =====${NC}"
...
if ! grep -q "flat2pojo-parent" pom.xml 2>/dev/null; then
    echo -e "${RED}Error: This doesn't appear to be the flat2pojo project root${NC}"
    echo "Expected to find 'flat2pojo-parent' in pom.xml"
    echo "Current directory: $(pwd)"
    exit 1
fi
