...
public final class PojoDiff {
    ...
    public static final class Config {
        ...
        // ---- Leaf controls
        /** Treat this Java type as a leaf (scalar) during type inference. */
        public Config leafType(Class<?> type) { extraLeafTypes.add(type); return this; }
        /** Treat all types under this package prefix as leaves during type inference. */
    ...
    public static TypeHints inferTypeHints(Class<?> rootType, ObjectMapper mapper, Config userCfg) {
        ...
        buildHints("", root, hints, mapper, sc, new HashSet<>(), userCfg);
        return hints;
    }

    private static void buildHints(String basePtr, JavaType jt, TypeHints hints,
                                   ObjectMapper mapper, SerializationConfig sc, Set<JavaType> seen, Config userCfg) {
        if (jt == null) return;
        ...
        for (BeanPropertyDefinition prop : props) {
            ...
            hints.put(ptr, pt);
            buildHints(ptr, pt, hints, mapper, sc, seen, userCfg);
        }

        // Fallback for immutable interfaces
        if (!gotAny && jt.isInterface()) {
            collectInterfaceAccessors(jt.getRawClass(), mapper).forEach((propName, propType) -> {
                ...
                hints.put(ptr, un);
                buildHints(ptr, un, hints, mapper, sc, seen, userCfg);
            });
        }
    }
    ...
    /** For immutable interfaces: collect property names from abstract accessors (honors @JsonProperty). */
    private static Map<String, JavaType> collectInterfaceAccessors(Class<?> iface, ObjectMapper mapper) {
        ...
        for (Method m : iface.getMethods()) {
            ...
            if (jp != null && !jp.value().isEmpty()) {
                ...
                if (mn.startsWith("get") && mn.length() > 3) {
                    ...
                    // Immutables style: method name IS the property name
                    name = mn;
                }
            }
