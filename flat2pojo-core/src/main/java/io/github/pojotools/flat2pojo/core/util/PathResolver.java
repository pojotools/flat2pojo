...
/**
 * Encapsulates all path-related operations with a specific separator.
 * Eliminates primitive obsession by bundling the separator with path operations.
 */
public record PathResolver(String separator) {
  ...
  /**
   * Gets the portion of a path after a prefix.
   * Example: tailAfter("a/b/c", "a") returns "b/c"
   */
  ...
  /**
   * Gets the final segment of a path.
   * Example: getFinalSegment("a/b/c") returns "c"
   */
  ...
  /**
   * Builds a path prefix by appending the separator.
   * Example: buildPrefix("a/b") returns "a/b/"
   */
   ...
   * Example: stripPrefix("a/b/c", "a/b/") returns "c"
   */
  ...
  /**
   * Checks if a relative suffix is under any child list of the parent.
   * Example: For parent "definitions", checks if suffix "modules/name" starts with "modules/"
   */
  public boolean isSuffixUnderAnyChild(final String suffix, final String parentPath, final Set<String> allListPaths) {
